...
INIT PROCEDURE __initHbQtQmlResources()
   QResource():registerResource_1( hbqtres_hbqtqmlparts() )
   RETURN


FUNCTION JustToLinkEverything()
   RETURN NIL
...
FUNCTION __hbqtActivateBarcodeReader( bBlock )

   IF ! HB_ISOBJECT( s_barcodeReader )
      WITH OBJECT s_barcodeReader := HbQtBarcodeReader():new():create( __hbqtAppWidget() )
         IF HB_ISBLOCK( bBlock )
            :barcodeFoundBlock := bBlock
         ENDIF
      ENDWITH
